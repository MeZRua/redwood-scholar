...
function loadPageDataForLocale(locale: string | undefined): HomePageLocaleData {
  ...
  const bioContent = getMarkdownContent('bio.md', locale);
  const allPublications = parseBibTeX(getBibtexContent('publications.bib', locale), locale);
  ...
  return {
    ...
    allPublications,
    description: stripMarkdown(bioContent.split('\n').filter(Boolean).slice(0, 2).join(' ')),
  };
}
